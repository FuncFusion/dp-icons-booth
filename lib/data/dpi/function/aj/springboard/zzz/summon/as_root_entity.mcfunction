...
data modify storage animated_java:temp entry.data.blueprint_id set value "dpi:aj/springboard"
data modify storage animated_java:temp entry.data.rig_hash set value "374443950caa7dbdeba07ecf9d04224d5e29aad84c67c1a9af6f6013b01b628b"
tp @s ~ ~ ~ ~ ~
execute on passengers if entity @s[tag=dpi.aj.springboard.node.bottom] run function dpi:aj/springboard/zzz/summon/as_node/bottom
data modify storage animated_java:temp entry.data.uuids append from storage animated_java:gu out
data modify storage animated_java:temp entry.data.uuids_by_name.bottom set from storage animated_java:gu out
execute on passengers if entity @s[tag=dpi.aj.springboard.node.springs] run function dpi:aj/springboard/zzz/summon/as_node/springs
data modify storage animated_java:temp entry.data.uuids append from storage animated_java:gu out
data modify storage animated_java:temp entry.data.uuids_by_name.springs set from storage animated_java:gu out
execute on passengers if entity @s[tag=dpi.aj.springboard.node.top] run function dpi:aj/springboard/zzz/summon/as_node/top
data modify storage animated_java:temp entry.data.uuids append from storage animated_java:gu out
data modify storage animated_java:temp entry.data.uuids_by_name.top set from storage animated_java:gu out
function dpi:aj/springboard/zzz/set_default_pose
# Data Manager: Write
function animated_java:global/data_manager/write with storage animated_java:temp args
execute if data storage animated_java:temp args.variant run function dpi:aj/springboard/zzz/summon/zzz/variant_arg/no_variants_warning
execute if score #success aj.i matches 0 run return fail
execute if data storage animated_java:temp args.animation run function dpi:aj/springboard/zzz/summon/animation_arg/process with storage animated_java:temp args
execute if score #success aj.i matches 0 run return fail
