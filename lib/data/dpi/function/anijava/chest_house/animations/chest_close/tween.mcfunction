...
tag @s add dpi.anijava.chest_house.animation.chest_close.playing
$scoreboard players set @s aj.tween_duration $(duration)
$scoreboard players set @s aj.chest_close.frame $(to_frame)
scoreboard players operation #this aj.i = @s aj.tween_duration
tag @s add aj.transforms_only
execute at @s run function dpi:anijava/chest_house/animations/chest_close/zzz/apply_frame {frame: 0}
$execute at @s run function dpi:anijava/chest_house/animations/chest_close/zzz/apply_frame {frame: $(to_frame)}
