...
data modify storage animated_java:temp entry.data.blueprint_id set value "dpi_aj:chest_house"
data modify storage animated_java:temp entry.data.rig_hash set value "4a15afe08b26c8cabbd6fbd2edb423594be99a0bd1b5f3a90ff4b409052e81ca"
tp @s ~ ~ ~ ~ ~
execute on passengers if entity @s[tag=dpi_aj.chest_house.node.lid] run function dpi_aj:chest_house/zzz/summon/as_node/lid
data modify storage animated_java:temp entry.data.uuids append from storage animated_java:gu out
data modify storage animated_java:temp entry.data.uuids_by_name.lid set from storage animated_java:gu out
execute on passengers if entity @s[tag=dpi_aj.chest_house.node.chest_house] run function dpi_aj:chest_house/zzz/summon/as_node/chest_house
data modify storage animated_java:temp entry.data.uuids append from storage animated_java:gu out
data modify storage animated_java:temp entry.data.uuids_by_name.chest_house set from storage animated_java:gu out
function dpi_aj:chest_house/zzz/set_default_pose
# Data Manager: Write
function animated_java:global/data_manager/write with storage animated_java:temp args
execute if data storage animated_java:temp args.variant run function dpi_aj:chest_house/zzz/summon/zzz/variant_arg/no_variants_warning
execute if score #success aj.i matches 0 run return fail
execute if data storage animated_java:temp args.animation run function dpi_aj:chest_house/zzz/summon/animation_arg/process with storage animated_java:temp args
execute if score #success aj.i matches 0 run return fail
